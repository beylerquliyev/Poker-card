...
export function Card() {
    ...
    useEffect(() => {
        async function ResultData() {
            ...
            let response2 = await response.json()
            setData(response2.cards[0].image)
            console.log(response2.cards[0].image)
        }
        ResultData()
        return () => {
            ResultData()
    ...
    function Pushdata() {
        ResultData(); // Wait for data to be fetched
console.log(data)
        if ((person1.length === 0 || person1.length === 1) && person1.length < 3 && data) {
            ...
            <div className={Style.Person1Div}>
                <div className={Style.CardTextDiv} >
                    ...
                    <div className={Style.CardDiv} >
                        {person1.map((i, index) => {
                            return <img key={index} className={Style.cards} src={i} alt="" />
                        })}
                    </div>
                </div>
